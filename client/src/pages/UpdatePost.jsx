...
export default function UpdatePost() {
  ...
  const [authorized, setAuthorized] = useState(true); // New state
  ...
  useEffect(() => {
    const fetchPost = async () => {
      try {
        ...
        // Check permission
        if (
          data.posts[0].userId !== currentUser._id &&
  ...
  if (!authorized) {
    return (
      <div className="p-3 max-w-3xl mx-auto min-h-screen flex items-center justify-center">
        <Alert color="failure">You are not allowed to update this post.</Alert>
      </div>
    );
  }

  if (!formData) return null; // Prevent premature rendering
